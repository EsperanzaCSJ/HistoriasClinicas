<div id = "sidebar">
	<ul id = "menu" class = "nav menu">
		<li><a href = "home.php"><i class = "glyphicon glyphicon-home"></i> Inicio</a></li>
		<li><a href = "paciente.php"><i class = "glyphicon glyphicon-user"></i> Agregar Paciente</a></li>
		<li><a href = "rehabilitation.php"><i class = "glyphicon glyphicon-book"></i> Agregar Historia</a></li>
		<!-- <li><a href = ""><i class = "glyphicon glyphicon-book"></i> Reportes</a>
					<ul>
						<li><a href = "#"><i class = "glyphicon glyphicon-print"></i> Pacientes del día</a></li>
						<li><a href = "#"><i class = "glyphicon glyphicon-print"></i> Recipe</a></li>
					</ul>
		</li> -->
		<li><a href = "view_rehabilitation_record.php"><i class = "glyphicon glyphicon-search"></i> Ver historia</a></li>
		<?php
		// include 'secciones.php';
		?>
</div>